...
const BadgeAccoladeModel = require('../models/badgeAccoladesModel');

// Handle GET request for all __badge
exports.getAllBadges = async (req, res) => {
    try {
        const __badge = await BadgeAccoladeModel.findAll();
...
exports.createBadge = async (req, res) => {
    try {
        const new__badge = new BadgeAccoladeModel(req.body);
...
exports.updateBadge = async (req, res) => {
    try {
        ...
        const __badge = await BadgeAccoladeModel.findByPk(req.params.id);
        if (__badge) {
...
exports.deleteBadge = async (req, res) => {
    ...
    try {
        const badge = await BadgeAccoladeModel.findByPk(badgeId);
        if (badge) {
            await badge.destroy();
...
exports.getBadgesById = async (req, res) => {
    const { id } = req.query;
    try {
        const badges = await BadgeAccoladeModel.findAll({
            where: {
                id: id
            }
        });
