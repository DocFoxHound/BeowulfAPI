...
- Auto-provisioning: when `VOICE_SESSIONS_AUTO_MIGRATE=true` the API will create the `voice_channel_sessions` table on first access if it does not already exist (it will not modify existing schemas).
